...
const createStudentComponent = (studentObject) => {
    return `
    <div class="student">
    <h1 class="xx-large ${studentObject.studentStatus}">${studentObject.name}</h1>
...
for (const student of students) {
    ...
    if (student.score >= 60) {
        student.studentStatus= "passing"
        studentComponent = createStudentComponent(student)
    } else {
        student.studentStatus= "failing"
        studentComponent = createStudentComponent(student)
    }
    studentHTML += studentComponent
}

// const studentHTML= ""
// //  const studentHTML is our container (variable) to hold ALL the snippets of HTML for all the students in the array above. It will be how all the students are seen at once on the DOM.

// for (const student of students) {
//     let studentComponent = ""
//     // let studentComponent is our container to hold EACH HTML snippet for each student (createStudentComponent)
//     if (student.score >= 60) {
...
//     }
//     studentHTML += studentComponent
//     // this is adding each HTML snippet for each student to the studentHTML variable to display them all at once on the DOM. The += is what builds it up instead of only displaying one student at a time.
// }


// ***** CODE FROM CLASS*****
// It takes four parameters: name, subject, info, score. First we start off with a conditional statement that checks whether the score is above or equal to 70 (a passing score) and if not, a failing score. We're setting those values to strings so we can use interpolation to set the class of the h1 to either passing or failing. Then, we return a string of HTML that takes the arguments we have passed in to the function.
...
// note from Eddy: I love you Shawna!!!!! Kissy moos! You're doing better than you think!